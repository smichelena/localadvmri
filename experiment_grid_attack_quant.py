...
width_loc = 10
n_locs = (8,8)
exp_name = "exp_grid_attack_quant_%s_width%d"%(mask_name,int(width_loc))
...
for i_batch, batch in enumerate(test_loader):
    ...
    for noise_rel in Noise_Rel:
        ...
        if noise_rel > 0:
            Yadv = spatial_grid_attack(Y,noise_rel,width=width_loc,n_locs=n_locs,iter=attack_iter)
            Xadv = reconstruct_full(Yadv,noise_rel)[0]
            ...
            Xadv = Xadv[idx]
            Pert = Yadv - Y
            Rho0 = reconstruct_full(Pert,noise_rel)[0] # reconstruction artifact for zero measurements
        else:
            Yadv = Y
            Xadv = Xrec
