...
from operators import Fourier, TVAnalysisPeriodic, rotate_real, proj_l2_ball, im2vec
...
OpA = Fourier(mask)
OpTV = TVAnalysisPeriodic((n,n),device=config.device)
...
def perturb(y0,noise_rel,loss=complexMSE,iter=30,init=None,method="l1"):
    """
    y0 has shape (...,2,m)
    """
    if noise_rel == 0.0:
    ...
    yadv = y0.clone().detach() + (adv_init_fac/np.sqrt(np.prod(y0.shape[-2:]))) * torch.randn_like(y0)
...
class LocalLoss:
    def __init__( self, shape, loc, width, loss=complexMSE, device="cpu" ):
        ...
        self.locmask = (dist2 <= (width/2.)**2).to(device)
...
def spatial_grid_attack( y0, noise_rel, width=1, n_locs=(1,1), iter=20, method="l1" ):
    device = y0.device
    shape = OpA.n
    if method == "l1":
    ...
    for ii in range(n_locs[0]):
        for jj in range(n_locs[1]):
            print("Perturbing at location\t(%d,%d) / (%d,%d)." %((ii+1,jj+1)+n_locs) )
            loc = grid[ii,jj]
            locLoss = LocalLoss(shape,loc,width,device=device)
            Yadv[ii,jj] = perturb( y0, noise_rel, loss=locLoss, iter=iter, init=init, method=method )
    return Yadv
