...
from operators import Fourier, TVAnalysisPeriodic
...
# should be <= noise_lvl
err = (OpA(Xhat) - Y).flatten(start_dim=1).norm(dim=-1)
...
for im_no, ax in enumerate(axs):
    ...
    plot_complex(Xhat[im_no],ax=ax[1])
    plot_complex(R[im_no],ax=ax[2],vmax="max")
if method == "l1":
    plt.suptitle("Reconstruction demo using %d iterations"%(rec_iterations))
else:
    plt.suptitle("Reconstruction demo using DNN")
path_fig = os.path.join(config.path_to_results,"demo_reconstruction.pdf")
